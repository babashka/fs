...
(deftest path-test
  (testing "single arg"
    (is (= "foo" (str (fs/path "foo"))))
    (is (= "/foo" (str (fs/path "/foo")))))
  (testing "two args"
    (is (= (str "foo" fs/file-separator "bar") (str (fs/path "foo" "bar"))))
    (is (= "/bar" (str (fs/path "foo" "/bar")))))
  (testing "three args"
    (is (= (str "a" fs/file-separator "b" fs/file-separator "c")
           (str (fs/path "a" "b" "c"))))))
...
(deftest parent-test
  (is (= "foo/bar" (str (fs/parent "foo/bar/baz"))))
...
(deftest normalize-test
  (is (= "a/b" (str (fs/normalize "a/b/c/.."))))
  (is (= "a/b/c" (str (fs/normalize "a//b//c")))))

(deftest relativize-test
  (is (= "c/d" (str (fs/relativize "a/b" "a/b/c/d"))))
  (is (= "../.." (str (fs/relativize "a/b/c/d" "a/b")))))
...
(deftest predicates-test
  (with-tmp [d]
    (let [f (str (fs/path d "file.txt"))]
      (fs/write-bytes f (.from js/Buffer "hello"))
      (testing "exists?"
        (is (fs/exists? d))
        (is (fs/exists? f))
  ...
  (with-tmp [d]
    (let [f (str (fs/path d "f.txt"))]
      (testing "write-bytes / read-all-bytes"
        (fs/write-bytes f (.from js/Buffer "hello"))
        (is (= "hello" (.toString (fs/read-all-bytes f)))))
...
(deftest copy-test
  (with-tmp [d]
    (let [src (str (fs/path d "src.txt"))
      ...
      (fs/write-bytes src (.from js/Buffer "data"))
      (fs/copy src dst)
      (is (= "data" (.toString (fs/read-all-bytes dst)))))))
...
(deftest delete-tree-test
  (with-tmp [d]
    (let [sub (fs/path d "sub")]
      ...
      (fs/write-bytes (str (fs/path sub "f.txt")) (.from js/Buffer "x"))
      (fs/delete-tree sub)
...
(deftest sym-link-test
  (with-tmp [d]
    (let [target (str (fs/path d "target.txt"))
      ...
      (fs/write-bytes target (.from js/Buffer "hi"))
...
(deftest glob-test
  (with-tmp [d]
    (fs/write-bytes (str (fs/path d "a.clj")) (.from js/Buffer ""))
    (fs/write-bytes (str (fs/path d "b.clj")) (.from js/Buffer ""))
    (fs/write-bytes (str (fs/path d "c.txt")) (.from js/Buffer ""))
    (fs/create-dir (fs/path d "sub"))
    (fs/write-bytes (str (fs/path d "sub" "d.clj")) (.from js/Buffer ""))
...
(deftest walk-file-tree-test
  (with-tmp [d]
    (fs/write-bytes (str (fs/path d "f1.txt")) (.from js/Buffer ""))
    (fs/create-dir (fs/path d "sub"))
    (fs/write-bytes (str (fs/path d "sub" "f2.txt")) (.from js/Buffer ""))
    (let [visited (atom [])]
...
(deftest last-modified-time-test
  (with-tmp [d]
    (let [f (str (fs/path d "f.txt"))]
      (fs/write-bytes f (.from js/Buffer "x"))
      (let [t (fs/last-modified-time f)]
        (is (instance? js/Date t))
...
(deftest gzip-gunzip-test
  (with-tmp [d]
    (let [src (str (fs/path d "data.txt"))
          gz  (str (fs/path d "data.txt.gz"))]
      (fs/write-bytes src (.from js/Buffer "hello world"))
      (let [out (fs/gzip src {:dir d})]
        (is (fs/exists? out))
        (let [extracted (fs/gunzip out d {:replace-existing true})]
          (is (= "hello world" (.toString (fs/read-all-bytes extracted)))))))))
